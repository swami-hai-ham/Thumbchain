  ...
  const getData = async () => {
    try {
      const response = await axios.get<Response[]>(
        `${BACKEND_LINK}/v1/user/task/${params.id}`,
        {
          headers: {
  ...
  return (
    <div className="w-full border-border border-l-2 h-screen flex justify-start items-center flex-col">
      <div className="flex justify-between items-center w-full h-1/6">
        <Link
          href="/thumbnail"
          className="text-foreground text-sm hover:scale-110 transition-all duration-200 font-poppins m-10 p-4 rounded-xl bg-border"
        >
          {"<  Back"}
        </Link>
        <button
          onClick={getData} // Retry on click
          className="bg-background flex justify-center items-center hover:scale-110 transition-all duration-200 text-foreground text-sm gap-2 m-10 p-5 rounded-xl"
        >
          Retry
          <svg
      ...
      {loading ? (
        <div className="flex space-x-2 justify-center items-center bg-background absolute top-2/4">
          <div className="size-5 bg-foreground rounded-full animate-bounce [animation-delay:-0.15s]"></div>
          <div className="size-5 bg-foreground rounded-full animate-bounce"></div>
          <div className="size-5 bg-foreground rounded-full animate-bounce [animation-delay:-0.3s]"></div>
        </div>
      ) : (
        <BarChart
