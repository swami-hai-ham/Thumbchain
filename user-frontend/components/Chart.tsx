...
const BarChart: React.FC<BarChartProps> = ({ data, labels, title, url }) => {
  ...
  return (
    <div
      className="bg-background p-4 rounded shadow h-1/2 w-full m-24"
      ref={divRef}
    >
      <div className="flex items-end h-full w-full justify-center">
        {data.map((value, index) => (
          <div
            key={index}
            className="flex flex-col items-center size-full justify-end gap-2"
          >
            <img
              src={url[index]}
              className="thumbnail h-32 w-2/4 object-contain opacity-0"
              alt=""
            />
            <div className="bg-chart-2 rounded-t w-3/4 bar opacity-0 flex justify-center items-center"></div>
            <div className="count opacity-0 font-poppins text-white text-center p-1 text-lg">
              {value}
            </div>
          </div>
        ))}
