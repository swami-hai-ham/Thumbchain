...
const SideNav = (props: Props) => {
  const { toast } = useToast()
    ...
    useGSAP(() => {
        ...
        gsap.fromTo([q('.sideNavLink'), q('.balance'), q('.payout')], {autoAlpha: 0, x: -100}, {x: 0, autoAlpha: 1, duration: 0.5, stagger: 0.2})
    },
    {scope: navRef})
    return (
        <div className='h-screen w-1/4 flex justify-center items-start flex-col gap-12 ml-28' ref={navRef}>
            ...
            <Button variant="secondary" className='text-xl rounded-xl text-bg font-montserrat p-4 py-5 hover:text-foreground hover:bg-primary payout' onClick={handleWithDraw}>Withdraw</Button>
        </div>
    )
}
